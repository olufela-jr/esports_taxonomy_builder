...
import type { RuleSet, RuleSetDraft } from '@/data/store';
...
export function RuleSetEditor({ existing, justCreated, onCreate, onUpdate, onDelete, onSaved, onClose }: { existing: RuleSet | null; justCreated?: boolean; onCreate: (draft: RuleSetDraft) => Promise<RuleSet>; onUpdate: (id: string, draft: RuleSetDraft) => Promise<void>; onDelete: (id: string) => Promise<void>; onSaved: (id: string) => void; onClose: () => void }) {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      await onUpdate(existing.id, draft);
      setSaved(true);
    } catch (cause) {
