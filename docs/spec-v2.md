...
Version 2, 22 September 2026. Source of truth is the Claude Doc; this file is the exported copy for the repo.
...
Where this spec departs from the scope-change brief, it says so in place and explains why. There are four such departures, all marked Proposed: segment references by `id` instead of `key`, a runtime guard that stops unresolved Rules reaching the engine, UTM mappings held per Rule rather than per Rule Set, and a literal-value UTM source.
...
| --- | --- | --- | --- | --- |
| G0 Spec review | Every Proposed decision (D24 to D33) is accepted or overruled, and the four departures from the brief are acknowledged | Decisions log with no Proposed entries | Fela | In progress |
