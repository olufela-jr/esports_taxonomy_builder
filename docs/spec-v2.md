# Campaign Naming Taxonomy Tool: Product Spec v2

Version 2, 22 September 2026, with the Gate G0 rulings of 24 September 2026 applied. Source of truth is the Claude Doc; this file is the exported copy for the repo.

## How to read this spec

Version 2 adds a hierarchical build flow (a child Rule inherits leading segments from a parent Rule) and UTM tracking URL generation, and supersedes `docs/spec.md` v1. Part A is the product view for anyone; Part B is the technical contract for whoever builds it; Part C covers operations and delivery. The decisions log and the pending list sit at the end so they can be scanned without reading the rest.

Every non-trivial statement carries one of five statuses, used consistently in the log:

| Status | Meaning |
| --- | --- |
| Settled | Decided in `CLAUDE.md`, `docs/spec.md` v1 or the scope-change brief, and carried forward unchanged |
| Reversed | Previously settled, changed by the scope-change brief |
| Proposed | A recommendation made in this spec to close a gap the source files leave open; adopt unless someone objects |
| Pending (client) | Needs a client answer before the affected part is built |
| Open | Not settled by the source files and not a client question; needs an internal decision |

Where this spec departs from the scope-change brief, it says so in place and explains why. There are four such departures, all acknowledged at Gate G0 on 24 September 2026: segment references by `id` instead of `key`, a runtime guard that stops unresolved Rules reaching the engine, UTM mappings held per Rule rather than per Rule Set, and a literal-value UTM source.

## Planning phase and gates

Building starts only after five gates pass, and each gate names the evidence that closes it, so "planned enough" is a checkable fact rather than a feeling. The gates are ordered, but their work overlaps: the client question pack goes out for Gate 1 on day one, and the dry run for Gate 2 starts on whatever sample data arrives first, because waiting on the client is the main way a planning phase turns into idle time.

```mermaid
flowchart LR
  G0[G0 Spec review] --> G1[G1 Model questions<br/>answered or defaulted]
  G1 --> G2[G2 Paper dry run<br/>on real names]
  G2 --> G3[G3 Release slicing<br/>and docs updated]
  G3 --> G4[G4 Build readiness]
  G4 --> B[Build R1]
  G0 -.->|starts in parallel| G2
```

Read it as: G0 and G1 are decisions, G2 is the only gate that produces evidence about the client's real data, G3 and G4 are housekeeping that stops the build starting from stale documents.

| Gate | Exit criteria | Evidence | Owner | Status |
| --- | --- | --- | --- | --- |
| G0 Spec review | Every Proposed decision (D24 to D33) is accepted or overruled, and the four departures from the brief are acknowledged | Decisions log with no Proposed entries | Fela | Passed 24 Sep 2026 |
| G1 Model-changing questions | P1, P5, P7 and P12 answered by the client, or the reply deadline passes and the spec defaults are confirmed in writing | Answers recorded against each question; deadline set (O10) | Fela with client | Not started |
| G2 Paper dry run | The client's real conventions written as Rules with parent links and one UTM mapping; a sample of real names (target 200 per Rule) checked against them; pass rate and every failure class recorded with a decision (rename, relax the Rule, or filter) | Dry-run sheet and failure-class list | Fela | Not started |
| G3 Release slicing and docs | Releases agreed (recommended R1 hierarchy and UTMs, R2 batch build, R3 Stage 2); acceptance criteria assigned per release; CLAUDE.md, docs/spec.md and the briefing updated per the change list | Three updated documents; per-release acceptance list | Fela | Not started |
| G4 Build readiness | Migration checklist steps 1 to 7 status confirmed; O2 environments, O3 CI host and O4 support owner decided; remaining Pending questions carry accepted defaults | Checklist with status; no Open item blocking R1 | Fela | Not started |

### Planning activities

- [ ] Send the client question pack: P1, P5, P7 and P12 first, P2 to P11 and P13 as a second block, each with the spec default and a reply deadline
- [ ] Request sample exports: distinct campaign and ad group or ad set names per platform, ideally from the BigQuery tables each Rule's `source` will point at, so the dry run also tests the Stage 2 routing
- [ ] Draft the client's Rules in a spreadsheet, one tab per Rule, segments in order with allowed values, parent links marked
- [ ] Run the dry run and record failure classes and their decisions
- [ ] Decide release slicing and write per-release acceptance criteria
- [ ] Apply the document change list to CLAUDE.md, docs/spec.md and the briefing
- [ ] Confirm migration checklist status with whoever holds the prototype
- [ ] Decide O2, O3 and O4

### Boundaries and risks of the planning phase

Planning produces no production code. A throwaway script for the dry run is fine, but anything reusable waits for R1, so the dry run does not quietly become an unreviewed second implementation of the engine, which D1 forbids.

Three things can go wrong. Client silence past the deadline: the defaults stand and G1 passes on written confirmation from Fela, not on the client. Sample data unavailable: the dry run runs on names pasted from the platform UI, smaller but still real. A low pass rate in the dry run: this is the planning phase working, not failing; it forces the rename-versus-relax decision now instead of after the checker ships and every ad group shows red.

## Part A: Product view

### What the product does

The tool lets a marketing operations team define how paid media entities must be named, produce names that are correct by construction, and find names already in use that break the convention. Version 2 adds two things the client asked for: an ad group name is built under a named campaign and carries that campaign's leading segments, and each build can output a full tracking URL with UTM parameters. These are one feature, because `utm_campaign` comes from the campaign and `utm_content` from a lower level, so the tool must know which campaign an ad group sits under before it can write a URL.

The framing for stakeholders is unchanged. The builder prevents bad names and bad tracking values because it only outputs what the convention allows. The checker detects bad names after the fact; it cannot rename anything already live on a platform.

### Who uses it

Two roles exist in practice, although v1 does not separate them in permissions (roles are deferred, Settled):

- Convention owner: authors Rule Sets, Rules, segments, parent links and UTM mappings.
- Campaign builder or analyst: builds names and tracking URLs from a convention, and checks existing names.

### Concepts

- Rule Set: the container for one organisation's or one account's conventions.
- Rule: one complete naming convention for one target, for example "Google Campaigns" or "Google Ad Groups".
- Segment: one ordered part of a name, either a fixed list of values (enum) or bounded free text (freeform).
- Parent link (new): a Rule may name another Rule in the same Rule Set as its parent and inherit some of its leading segments. The inherited part is always read from the parent, never copied, so the two cannot drift apart.
- UTM mapping (new): says which built value feeds each UTM parameter for a Rule, and which base URL to use.

```mermaid
flowchart TD
  RS[Rule Set<br/>Acme UK Paid Media] --> C[Rule: Google Campaigns<br/>type_market_objective]
  RS --> MC[Rule: Meta Campaigns]
  C -->|parent of| AG[Rule: Google Ad Groups<br/>inherits type_market<br/>adds targeting_audience]
  AG --> U[UTM mapping<br/>+ base URL]
  U --> URL[Tracking URL]
```

Read it as: the ad group Rule does not store type or market itself; it points at the campaign Rule for them. Editing the campaign Rule's market values changes what the ad group Rule accepts, which is intended and is also the main new risk (see Part C).

### Worked example

Campaign Rule segments are `type`, `market`, `objective` with delimiter `_`. Ad group Rule inherits `type` and `market` and adds `targeting`, `audience`. A user builds campaign `perf_uk_sales`, then chooses "Build ad group under this campaign". The ad group form arrives with `perf` and `uk` filled and locked; the user picks `broad` and types `runners`. The tool outputs ad group `perf_uk_broad_runners` and, if the ad group Rule has a UTM mapping, a URL such as `https://shop.example.com/sale?utm_source=google&utm_medium=cpc&utm_campaign=perf_uk_sales&utm_content=perf_uk_broad_runners`.

Note what the example makes visible: the ad group name repeats two campaign segments but not `objective`. Whether inherited segments must be a leading run of the parent (as here) is Proposed below. Whether the client's existing ad group names actually repeat campaign segments today is Pending (client), and it matters: if they do not, every existing ad group will fail the checker once the new Rule is live.

### Scope

In scope for v2:

- Authoring: parent links between Rules in one Rule Set, with guards against cycles, missing segments and invalid combined ordering.
- Build: a parent step for child Rules (paste a parent name, or continue from a parent just built in the session); tracking URL output with validated UTM values.
- Build, Batch mode (added 22 Sep): generate every combination of chosen segment values for a Rule as a CSV, with a live count and a row cap (Journey 4).
- Check: child Rules validate against their combined (resolved) segments. Everything else about checking is unchanged.

Out of scope, restated so nobody reopens them:

- Cross-level validation: checking that an ad group's inherited prefix matches a real campaign. Needs Stage 2 data (Settled by brief).
- Hierarchy derived from warehouse parent relationships (Settled by brief).
- Checking UTMs already live in landing URLs: Stage 2 item (Settled by brief).
- Picking a parent from a list of live campaigns: depends on Stage 2 data; v2 uses paste or in-session continuation (Proposed).
- Scheduled scanning, roles, versioning, approval workflows, exceptions list, server-side CSV processing, natural-language input, writing back to ad platforms (Settled).

### Journey 1: Author a parent and child convention

The owner opens Author with a Rule Set selected, creates "Google Campaigns" as today, then creates "Google Ad Groups". On the ad group Rule, a new Parent field lists the other Rules in the Rule Set. Choosing "Google Campaigns" shows the parent's segments as a checklist; the owner ticks the leading ones to inherit. Inherited segments appear read-only at the top of the segment list, labelled with their source Rule, and the owner adds the child's own segments beneath them. The delimiter is shown as the parent's and cannot be changed while a parent is set (Proposed, see decisions).

If the owner then tries to delete a parent segment that a child inherits, change the parent's delimiter, or delete the parent Rule itself, Author blocks the change and names the dependent Rules. Renaming a parent segment is safe, because children reference it by immutable id. Adding or removing enum values on the parent is allowed, with a warning that it changes what the child Rules accept.

The owner optionally adds a UTM mapping on the Rule that will output URLs: a source for each of the five parameters, a default base URL, and whether the base URL may be edited at build time.

### Journey 2: Build a campaign, then an ad group, then a URL

```mermaid
flowchart LR
  A[Build campaign] --> B[Copy name]
  B --> C[Build ad group<br/>under this campaign]
  P[Paste existing<br/>campaign name] --> D
  C --> D{Parent parses<br/>against parent Rule?}
  D -->|yes| E[Inherited segments<br/>filled and locked]
  D -->|no| X[Show violations,<br/>block continue]
  E --> F[Complete own segments]
  F --> G[Name + tracking URL]
```

There are two ways into a child build. The chained way starts from a parent just built: a "Build ad group under this campaign" action switches the persistent Rule context to the child and carries the parent name across. The direct way starts on the child Rule: the Parent step asks the user to paste the parent name, which the engine validates against the parent Rule. A parent name that fails validation stops the flow with the parent's violations shown; the user must fix or rebuild the parent first. This means an ad group cannot be built under a legacy, non-compliant campaign, which is correct for the round-trip guarantee but may frustrate users during a migration (Pending (client), see override question).

For a grandchild Rule (for example ads under ad groups), the parent step asks for the immediate parent's name only; that name already contains the grandparent's inherited segments.

The output panel shows the name, and when the Rule has a UTM mapping, the full tracking URL with each UTM value listed and validated individually. Copy buttons stay disabled until the name and every UTM value are valid.

### Journey 3: Check

Check works as in v1. Selecting a child Rule validates names against the child's combined segments, so an ad group name must carry valid inherited values in the right positions. It does not confirm that those values belong to a real campaign; that is cross-level validation and is out of scope. The pooled "All Rules" rollup includes child Rules like any other Rule.

### Journey 4: Batch build (Proposed, added 22 Sep 2026)

The requirement: once a Rule exists, the tool generates every allowed name from it, not one campaign at a time. Because segment positions are fixed, the output is the Cartesian product of the chosen values per segment (combinations), never reorderings (permutations). A name with the segments in a different order is invalid by definition.

Two things stop "every allowed name" being literal. Freeform segments have no finite value list, so the user supplies the values a batch should use for each one, or leaves it at a single value. And the full product grows fast: three enum segments of 3, 4 and 6 values plus one optional segment is 144 names, while six segments of 8 values is 262,144. In practice a batch is "these two markets, all three campaign types, this one objective", so the design makes a chosen subset the normal case and "select all" the ceiling.

How it works in Build:

- A Single or Batch toggle at the top of the Build workspace. Single is today's flow and stays the default.
- In Batch, each enum control becomes a multi-select with Select all; each freeform control takes one value per line, validated live as in Single; each optional segment offers include, omit or both.
- A live count shows how many names the current choices produce, before anything is generated.
- Generate is blocked above a hard cap (Proposed: 50,000 rows) with the count shown and a prompt to narrow the choices. The cap protects browser memory; the engine itself streams.
- Output is a preview table of the first 500 rows and a CSV download with one column per segment, the name, and the tracking URL when the Rule has a UTM mapping. Nothing is written to any ad platform (Settled out of scope) and nothing is checked for uniqueness against live data (Stage 2).

For a child Rule, Batch runs under one parent name; the parent step is unchanged. Generating ad groups across many campaigns at once multiplies the product again and needs a list of parent names as input; that is Open (O9), with a recommendation to defer to a second pass.

What the batch output is for decides its format, and the files do not say. If it is a reference list of permitted names, the CSV above is enough. If it is meant for bulk upload through a platform editor or bulk sheet, each platform expects its own columns, and that is a per-platform export feature, not a naming feature. Pending (client) as P12.
...
## Build order and document changes

The v2 work slots in after the migration checklist and before Stage 2 (Proposed), because it is browser-only and depends on Firestore and the `id` fields from migration steps 2 and 4. Order: engine types and `resolveRule` with tests; runtime guard; `checkRuleSet` and `dependentsOf`; Author parent UI; Build parent step and chaining; UTM types, `buildTrackingUrl` and tests; Author UTM panel; Build URL output; Playwright additions. Stage 2 then follows as specified, with the `resolveRule` change.

New acceptance criteria, added to the v1 list:

1. A child Rule authored in the UI drives Build and Check with no code change, and its names validate against its resolved segments.
2. Author rejects cycles, missing inherited segments, non-leading or optional inherited segments, delimiter mismatch and key collisions.
3. Deleting a parent segment or Rule with dependents is blocked; relabelling is not.
4. The parent, name and URL round-trip tests pass.
5. Every built URL's `utm_campaign` equals the built campaign name byte for byte.

### CLAUDE.md

- "Rules are independent: no inheritance, no cross-Rule validation" becomes "A Rule may inherit leading segments from a parent Rule in the same Rule Set, by reference, at build time. Cross-level validation is out of scope."
- The Do not line "Do not add cross-Rule inheritance or parent-child validation" becomes "Inheritance by reference at build time is in scope; cross-level validation is not."
- The last Do not bullet drops "Rules are independent (decided)" from its settled list.
- Data model essentials: add `parent?` and `utm?` to the Rule shape; note that references use segment `id`.
- The one rule: add "`compose` and `validate` take resolved Rules only; call `resolveRule` first" and "UTM logic lives in `@taxo/shared` too".
- Add the v2 build order after migration step 7.

### docs/spec.md

Replace it with the technical parts of this spec (Part B, Part C, decisions log, pending list). Specifically removed or changed from v1: the Terminology line on independence, the Firestore model intro and note "Rules are independent", the Out of Scope entry for cross-Rule inheritance, the engine types (extended), acceptance criteria (extended), build order (extended). Product view material can stay in this document; the repo spec keeps a one-paragraph purpose statement and links here.

### Client briefing

- Hierarchy diagram: redraw with the parent link from the campaign Rule to the ad group Rule, and replace the caption "changing one never affects another" with "a child Rule reads its leading segments from its parent, so changing the parent changes the child". Keep the diagram as a native Word table so labels stay editable by hand.
- Rule definition text: add the parent link in one sentence.
- New UTM section: what the tool outputs, the worked example URL from Part A, and the rule that `utm_campaign` is always the campaign name exactly.
- Add the client questions P1 to P11 as an appendix or a covering note, with P1, P5 and P7 flagged as needed first.
