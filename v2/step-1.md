...
- `packages/shared/src/engine.ts`:
  ...
  - `ResolveResult = { rule, errors }` and `resolveRule(rule, ruleSet)`. Recursive with a visited
    ...
    Checks in order: parent exists, no cycle, parent resolves, every id
...
- `packages/shared/src/engine.test.ts`: a `resolveRule` suite of 15 cases over a three-level
  ...
  id collision, an empty inherit list (allowed, resolves to the child's own segments), and a
  broken parent reported from the grandchild.
...
- An empty `inheritSegmentIds` is allowed and resolves to the child's own segments, with the
  delimiter check still applied. It was an error in the first cut; changed at review on
  24 September 2026.
...
- `docs/features/d34-child-batch.md` (child batch across parents) is untracked in the repo and
  depends on steps 1, 2, 5, 6 and the batch release. Its section 10 said npm workspaces; corrected
  to pnpm at review on 24 September 2026 and committed with these adjustments.
